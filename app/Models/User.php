...
class User extends Authenticatable
{
    ...
    public function relationPlan()
    {
        return $this->hasOneThrough(
            ...
            'id',
            'plan_id'
        )->select(['plans.*', 'subscriptions.stripe_status'])
    ...
    {
        // Use direct database query to prevent circular dependencies and timeout
        ...
        // Check Yokassa subscription if no regular subscription found
        $yokassaSub = \App\Models\Finance\YokassaSubscription::query()
            ->where('user_id', $this->id)
            ->whereIn('subscription_status', ['active', 'yokassa_approved'])
            ->first();
            
        if ($yokassaSub) {
            $plan = Plan::where('id', $yokassaSub->plan_id)->first();
            if (is_null($plan)) {
                return null;
            }
            
            $difference = $yokassaSub->updated_at->diffInDays(Carbon::now());
            
            if (in_array($plan->frequency, [FrequencyEnum::MONTHLY->value, FrequencyEnum::LIFETIME_MONTHLY->value])) {
                if ($difference < 31) {
                    return $plan;
                }
            } elseif (in_array($plan->frequency, [FrequencyEnum::YEARLY->value, FrequencyEnum::LIFETIME_YEARLY->value])) {
                if ($difference < 365) {
                    return $plan;
                }
            } else {
    ...
    public function businessCredits(): HasOne
    {
        return $this->hasOne(BusinessCredit::class);
    }
