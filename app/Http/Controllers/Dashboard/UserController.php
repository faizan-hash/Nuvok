...
class UserController extends Controller
{
    ...
    public function index()
    {
        ...
        // Skip expensive operations that can cause infinite loops
        // $ongoingPayments = self::prepareOngoingPaymentsWarning();
        // $tmp = PaymentProcessController::checkUnmatchingSubscriptions();
        
        $user = Auth::user();
        
        // Skip expensive operations that can cause infinite loops or timeouts
        $ongoingPayments = null;
        
        // Get team information more efficiently
        $team = $this->getTeamOptimized($user);

        // Optimize recent activity query with minimal data
        $recentlyLaunched = UserOpenai::query()
            ->select(['id', 'user_id', 'title', 'slug', 'created_at', 'updated_at', 'output'])
    ...
    /**
     * Optimized version of getTeam to prevent timeout issues
     */
    private function getTeamOptimized(User $user)
    {
        // Check if user already has a team
        $existingTeam = $user->myCreatedTeam;
        
        if ($existingTeam) {
            // Get plan seats safely without complex relationships
            $planAllowSeat = $this->getPlanAllowSeatOptimized($user);
            
            // Only update if different to avoid unnecessary queries
            if ($existingTeam->allow_seats != $planAllowSeat) {
                $existingTeam->update(['allow_seats' => $planAllowSeat]);
            }
        ...
        $allowSeats = $this->getPlanAllowSeatOptimized($user);
        
        return Team::query()->firstOrCreate([
            'user_id' => $user->id,
        ], [
            'name'        => $user->fullName(),
            'allow_seats' => $allowSeats,
        ]);
    }

    /**
     * Optimized method to get plan allow seat without complex relationships
     */
    private function getPlanAllowSeatOptimized(User $user): int
    {
        if ($user->isAdmin()) {
            return 100;
        }

        // Use a more efficient query to get active subscription
        $activeSub = DB::table('subscriptions')
            ...
            ->select('plans.plan_allow_seat')
            ->orderBy('subscriptions.created_at', 'desc')
            ->first();
            
        return $activeSub ? (int) $activeSub->plan_allow_seat : 0;
    }
    ...
    /**
     * Optimized version of getTeam for backward compatibility
     */
    public function getTeam(User $user)
    {
        return $this->getTeamOptimized($user);
    }

    /**
     * Optimized version to prevent timeout issues
     */
    public function prepareOngoingPaymentsWarning()
    {
        // Skip expensive operations that can cause timeouts
    ...
    {
        dd('UserController in Dashboard');
