...
class PlanAndPricingController extends Controller
{
    ...
    public function creditListPartial(Request $request): JsonResponse
    {
        ...
        if ($planId = $request->get('plan_id')) {
            ...
            // Use plan's business credits instead of user's current credits
            if ($plan) {
                $planBusinessCredits = $plan->getBusinessCreditsByPrice();
                $businessCredits = (object) [
                    'invoices' => $planBusinessCredits['invoices'],
                    'clients' => $planBusinessCredits['clients'],
                    'projects' => $planBusinessCredits['projects'],
                    'tasks' => $planBusinessCredits['tasks'],
                ];
            }
        } else {
            // Show user's current credits if no plan is specified
            $businessCredits = auth()->user()->businessCredits;
        }

        return response()->json([
            'html' => Cache::remember($cacheKey, 2, function () use ($plan, $businessCredits) {
