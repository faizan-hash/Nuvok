...
// dd('THIS IS THE RIGHT CONTROLLER FILE');
class AuthenticatedSessionController extends Controller
{
    ...
    {
        // Debug: Log login form view creation
        \Log::info('Login form created', [
    ...
    {
        // Debug: Log that we've reached the store method
        ...
        if ($plan = $request->get('plan')) {
            // Only redirect to payment if plan parameter has a value
            if (!empty($plan)) {
                \Log::info('Redirecting to payment page with plan', ['plan' => $plan]);
                return response()->json([
        ...
        \Log::info('Redirecting to dashboard without plan');
        return response()->json([
            'link' => '/dashboard/user',
        ]);
    }
