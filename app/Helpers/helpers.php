...
{
    // Use direct query to prevent circular dependencies
    $userId = auth()->id();
    
    $activeSub = \Laravel\Cashier\Subscription::query()
        ->where('user_id', $userId)
    ...
    // Check Yokassa subscription
    return \App\Models\Finance\YokassaSubscription::query()
        ->where('user_id', $userId)
...
{
    // Simplified version to prevent timeout
...
function checkIfTrial()
{
    ...
    return $subscription && $subscription->stripe_status === 'trialing';
}
...
function getSubscriptionName()
{
    ...
    $planId = $subscription->plan_id ?? $subscription->name;
...
function getSubscriptionDaysLeft()
{
    ...
    return $now->diffInDays($endsAt, false);
}
