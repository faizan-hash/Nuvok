...
class StreamService
{
    ...
    private function deepseekChatStream($chat_bot, $history, $main_message, $contain_images)
    {
        $total_used_tokens = 0;
        $output = '';
        $responsedText = '';
        ...
        if ($contain_images) {
            ...
            $driver = Entity::driver(EntityEnum::DEEPSEEK_CHAT);
        }

        return response()->stream(function () use ($driver, $chat_bot, $history, &$total_used_tokens, &$output, &$responsedText, $main_message, $contain_images) {
    ...
    private function deepseekOtherStream(Request $request, $chat_bot)
    {
        ...
        return response()->stream(function () use ($driver, $message_id, $title, $openai_id, $prompt, $history, $chat_bot) {
