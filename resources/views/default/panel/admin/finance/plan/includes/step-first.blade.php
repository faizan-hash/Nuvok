...
<div class="w-full space-y-7">
	...
	<!-- Business Credits Section -->
	<div class="row gap-y-7">
		<div class="col-12">
			<x-form-step
				class="mb-0"
				step="4"
				label="{{ __('Business Credits') }}"
			/>
			<div class="form-label mt-5">
				{{ __('Set the number of business credits included in this plan. Users will consume these credits when creating invoices, clients, projects, and tasks.') }}
			</div>
		</div>
		<div class="col-12 col-sm-6">
			<x-form.group
				label="{{ __('Invoice Credits') }}"
				tooltip="{{ __('Number of invoices that can be created with this plan') }}"
				error="plan.business_invoices_limit"
			>
				<x-form.stepper
					wire:model="plan.business_invoices_limit"
					type="number"
					step="1"
					min="0"
					placeholder="{{ __('Invoice Credits') }}"
				/>
			</x-form.group>
		</div>
		<div class="col-12 col-sm-6">
			<x-form.group
				label="{{ __('Client Credits') }}"
				tooltip="{{ __('Number of clients that can be created with this plan') }}"
				error="plan.business_clients_limit"
			>
				<x-form.stepper
					wire:model="plan.business_clients_limit"
					type="number"
					step="1"
					min="0"
					placeholder="{{ __('Client Credits') }}"
				/>
			</x-form.group>
		</div>
		<div class="col-12 col-sm-6">
			<x-form.group
				label="{{ __('Project Credits') }}"
				tooltip="{{ __('Number of projects that can be created with this plan') }}"
				error="plan.business_projects_limit"
			>
				<x-form.stepper
					wire:model="plan.business_projects_limit"
					type="number"
					step="1"
					min="0"
					placeholder="{{ __('Project Credits') }}"
				/>
			</x-form.group>
		</div>
		<div class="col-12 col-sm-6">
			<x-form.group
				label="{{ __('Task Credits') }}"
				tooltip="{{ __('Number of tasks that can be created with this plan') }}"
				error="plan.business_tasks_limit"
			>
				<x-form.stepper
					wire:model="plan.business_tasks_limit"
					type="number"
					step="1"
					min="0"
					placeholder="{{ __('Task Credits') }}"
				/>
			</x-form.group>
		</div>
	</div>
