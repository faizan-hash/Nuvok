...
textRotators?.forEach(textRotator => {
	...
	const firstSpan = items[activeIndex].querySelector('span');
	if (!firstSpan) return;

	textRotator.style.width = `${firstSpan.clientWidth}px`;
	...
	setInterval(() => {
		...
		textRotator.style.width = `${span.clientWidth}px`;
		activeItem.classList.add('lqd-is-active');
	}, timeout);
});
